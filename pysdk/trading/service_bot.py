...
class PositionState:
    ...
    is_against_market: bool
    pos_market_delta: float 
...
class TradingBot:
    def __init__(self, validator_mnemonic: str, pair: str, append: bool):
        ...
        self.curr_pos: Optional[PositionState] = None
    ...
    def run(self):
        while not self.connected:
            ...
            try:
                # create val only if mnenomic passed
                self.validator = (
                ...
                self.curr_pos.trader = self.sdk.address
                self.connected = True
            except BaseException as err:
                logging.error(err)
                time.sleep(10)

    def should_make_position(
        self, quote_needed_to_move_price: float, reserves: float, net_position: float
    ) -> bool:
    ...
    def close_position(self, pair: str):
        if self.curr_pos.has_positions:
            try:
                ...
                self.pos_dict.get("positions").append(
                    "CLOSED POSITION: " + str(positions_map)
                )
    ...
    def make_position(self, quote_to_move: int):
        ...
        try:
            ...
            self.pos_dict.get("positions").append(
                "OPENED POSITION: " + str(positions_map)
            )
    ...
    def find_index_quote(self) -> float:
        process = subprocess.Popen(
            "nibid q oracle exchange-rates", shell=True, stdout=subprocess.PIPE
        )
        process.wait()
    ...
    def parse_index_quote(self, data: bytes) -> float:
        ...
        if len(nibid_exchange_rates_json["exchange_rates"]) != 0:
            index_quote = float(
                nibid_exchange_rates_json["exchange_rates"][0]["exchange_rate"]
            )
        else:
    ...
    def find_mark_quote(self) -> Tuple[float, float, float, float]:
        process = subprocess.Popen(
            "nibid q perp markets", shell=True, stdout=subprocess.PIPE
        )
        process.wait()
        data, err = process.communicate()
        if process.returncode == 0:
    ...
    def parse_mark_quote(self, data: bytes) -> Tuple[float, float, float]:
        ...
        net_position = float(nibid_amm_json['amm']['total_long']) - float(
            nibid_amm_json['amm']['total_short']
        )
    ...
    def quote_needed_to_move_price(
        self, current_price: float, target_price: float, quote_reserve: float
    ) -> float:
...
def main():
    ...
    while True:
        ...
        quote_to_move_price = bot.quote_needed_to_move_price(
            current_price=mark_quote,
            target_price=index_quote,
            quote_reserve=quote_asset_reserve,
        )
        should_trade = bot.should_make_position(
            quote_to_move_price, quote_asset_reserve, net_position
        )

        if should_trade and not bot.curr_pos.has_positions:
            bot.curr_pos.size, bot.curr_pos.unrealized_pnl = bot.make_position(
                quote_to_move_price / 1000000
            )

        # is against market depends on market struct and positionstate struct
        if bot.curr_pos.has_positions:
            bot.curr_pos.is_against_market = bot.is_pos_against_market(
                bot.curr_pos.size, index_quote, mark_quote
            )
            bot.curr_pos.pos_market_delta = abs(
                (mark_quote - index_quote) * price_multiplier * bot.curr_pos.size
            )

        if (
            not should_trade
            and bot.curr_pos.is_against_market
            and bot.curr_pos.pos_market_delta > 0.1 * index_quote
        ):
            bot.close_position(pair=bot.curr_pos.pair)

        if (
            not bot.curr_pos.is_against_market
            and bot.curr_pos.unrealized_pnl > 0.1 * abs(bot.curr_pos.size)
        ):
            bot.close_position()
